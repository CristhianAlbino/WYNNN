...
app.post('/servicos-catalogo', authMiddlewareUsuario, async (req, res, next) => {
    try {
        ...
        // Se latitude e longitude forem fornecidas, use a busca por proximidade
        if (latitude !== null && longitude !== null && latitude !== undefined && longitude !== undefined) {
            // Adiciona um filtro de proximidade (ex: 50 km)
            // Certifique-se de que o modelo Prestador tem um índice 2dsphere no campo 'location'
            query = {
                'prestador_id.location': {
                    $nearSphere: {
                        $geometry: {
                            type: "Point",
            ...
            // Ordenar por proximidade é implícito com $nearSphere, mas pode-se adicionar um sort secundário
            // Não é necessário um sort explícito para $nearSphere, ele já retorna ordenado por distância
        }

        // 1. Buscar todos os serviços oferecidos e popular os dados básicos do prestador
        // Usamos aggregate para poder usar $lookup com o filtro geoespacial
        const servicosCatalogo = await ServicoOferecido.aggregate([
            {
                $lookup: {
            ...
            {
                $match: {
                    // Aplica o filtro de geolocalização aqui, se houver
                    ...query,
                    // Garante que o prestador tem uma localização definida se o filtro de proximidade estiver ativo
                    ...(latitude !== null && longitude !== null && latitude !== undefined && longitude !== undefined ? { 'prestadorInfo.location': { $exists: true } } : {})
                }
            },
            {
                $lookup: {
